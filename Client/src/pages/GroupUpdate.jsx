...
const GroupUpdate = () => {
  ...
  const initilSelectedUserList = selectedGroup.userList.map((user)=>{return {id:user.id ,name:user.name, email:user.email}})
  const [selectedUserList, setSelectedUserList] = useState(initilSelectedUserList)
  ...
  const handleAddUser = (user) => {
    return () => {
      ...
      setSelectedUserList([...selectedUserList, {id: user.id, name:user.name}])
    }
  }
  ...
  const handleSubmit = async (event) => {
    ...
    fetch('/api/groups/update',{
      ...
      body: JSON.stringify({id:selectedGroup.groupId ,name,description,userList})
    }).then((res)=>{
      if (res.status!==202){
        ...
        <Col className='text-center d-flex justify-content-center'>
          <Form onSubmit={handleSubmit}>
            <Form.Group className="mb-3">
              ...
              <Form.Control name="name" style={{width:'300px'}} placeholder={selectedGroup.name} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label>Group Description</Form.Label>
              <Form.Control name="description" as="textarea" placeholder={selectedGroup.description} style={{ height: '100px'}}/>
            </Form.Group>
            <Form.Group className="mb-3">
